...
const SurketController = {
  async createSurket(req, res) {
    ...
    try {
      ...
      const { nik, nama, tempat_lahir, tanggal_lahir, alamat, agama, keperluan } = req.body;
      console.log(nik, nama, tempat_lahir, tanggal_lahir, alamat, agama, keperluan, user_id);

      if (!nik || !nama || !tempat_lahir || !tanggal_lahir || !alamat || !agama || !keperluan || !user_id) {
        return res.status(400).json({ error: 'Harap lengkapi semua informasi surat keterangan.' });
      }

      const newSurket = await SurketModel.createSurket(nik, nama, tempat_lahir, tanggal_lahir, alamat, agama, keperluan, user_id);
