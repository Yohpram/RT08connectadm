...
async function createSurket(nik, nama, tempat_lahir, tanggal_lahir, alamat, agama, keperluan, token) {
  try {
    const response = await instance.post("/surket", {
     nik, nama, tempat_lahir, tanggal_lahir, alamat, agama, keperluan
    }, { headers: {
      'x-auth-token': token,
    }});
    return response.data;
